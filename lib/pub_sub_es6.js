  ...
  return function on(target, name, descriptor) {
    ...
    Object.defineProperty(target.constructor.prototype, "componentDidMount", {
      value: function value() {
        var scopeValue = _callChainMethods(this, options.scope);
        var actionName = _generateActionNameReact({ actionType: actionType, scopeValue: scopeValue });
        var uid = _generateUidReact({ target: target, scopeValue: scopeValue, reactInstance: this });
...
var debuggerConsole = function debuggerConsole() {
  ...
  config.enableDebugger && (_console = console).info.apply(_console, arguments);
};

function _generateUidReact(_ref) {
  var target = _ref.target,
      reactInstance = _ref.reactInstance,
      scopeValue = _ref.scopeValue;

  var reactUid = reactInstance._reactInternalInstance._debugID;
  var componentName = target.constructor.name;
  return scopeValue ? reactUid + "-" + componentName + "|" + scopeValue : reactUid + "-" + componentName;
}

function _generateActionNameReact(_ref2) {
  var actionType = _ref2.actionType,
      scopeValue = _ref2.scopeValue;
...
function _callChainMethods(object, string_chanin) {
  ...
  var memo = object;
  string_chanin.split(".").forEach(function (key) {
